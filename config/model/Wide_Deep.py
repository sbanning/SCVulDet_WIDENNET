...
class WIDEVULDET:
    ...
    def build_model(self):
        ...
        class_weights = compute_class_weight(class_weight='balanced', classes=[0, 1], y=self.labels)
        ...
        hidden1 = Dense(192, activation='relu')(norm_deep)
        hidden2 = Dense(320, activation='relu')(hidden1)

        concat = Concatenate(axis=-1)([norm_wide, hidden2])
        flatten = Flatten()(concat)
        h_final = Dense(224, activation='relu')(flatten)
        output = Dense(2, activation='softmax')(h_final)
        # output = Dense(1)(concat)

        self.model = Model(inputs=[input_deep, input_wide], outputs=[output])

        optimizer = Adam(learning_rate=0.0001) #1.8840040300e-4)
        optimizer = SGD(learning_rate= 0.0001) #1.8840040300e-4)
